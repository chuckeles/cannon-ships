...
/*
			...
			UnityEditor.GenericMenu.MenuFunction2 AddFieldSetNode = delegate(object f){
				var fieldWrapper = AddNode<ReflectedFieldNodeWrapper>(mousePos);
				fieldWrapper.SetField( (FieldInfo)f, ReflectedFieldNodeWrapper.AccessMode.SetField );
			};
*/
			//Macro Wrapper
			UnityEditor.GenericMenu.MenuFunction2 AddMacroNode = delegate(object m){
			...
			menu = EditorUtils.GetTypeSelectionMenu(typeof(SimplexNode), AddSimplexNode, menu);
			...
			foreach (var type in UserTypePrefs.GetPreferedTypesList(typeof(object))){
				...
				foreach (var m in methods.OrderBy(m => !m.IsStatic).OrderBy(m => m.IsSpecialName).OrderBy(m => m.DeclaringType != type) ){
					...
					menu.AddItem(new GUIContent(name, icon, null ), false, AddMethodNode, m);
				}
/*
				//Fields
				foreach (var f in type.GetFields(flags)){
					...
					var categoryName = "Functions/Reflected/" + type.FriendlyName() + "/Fields/";
					if (f.DeclaringType != type){ categoryName += "More/"; }

					var nameForGet = categoryName + (f.IsReadOnly()? "constant " + f.Name : "Get " + f.Name);
					menu.AddItem(new GUIContent(nameForGet, icon, null), false, AddFieldGetNode, f);

					if (!f.IsReadOnly()){
						var nameForSet = categoryName + "Set " + f.Name;
						menu.AddItem(new GUIContent(nameForSet, icon, null), false, AddFieldSetNode, f);
					}
				}
*/
			}
			...
			foreach (var pair in variables){
				foreach(var _bbVar in pair.Value){
					...
					menu.AddItem(new GUIContent("Variables/Blackboard/Get " + finalName, null, "Get Blackboard Variable"), false, ()=> {
					...
					menu.AddItem(new GUIContent("Variables/Blackboard/Set " + finalName, null, "Set Blackboard Variable"), false, ()=> {
			...
			foreach(var guid in projectMacroGUIDS){
				...
				if (macro != this){
					menu.AddItem(new GUIContent("MACROS/" + macro.name, null, macro.graphComments), false, AddMacroNode, macro);
				}
			}
		...
		protected override void OnDropAccepted(Object o, Vector2 mousePos){
			...
			UnityEditor.GenericMenu.MenuFunction2 AddMethodNode = (m)=>{
