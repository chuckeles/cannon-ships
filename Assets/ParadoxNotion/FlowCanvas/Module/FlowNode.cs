...
namespace FlowCanvas{
	...
	abstract public class FlowNode : Node, ISerializationCallbackReceiver {
		...
		void DoContextPortConnectionMenu(Port clickedPort, Vector2 mousePos, float zoomFactor){
			...
			//SimplexWrapper
			UnityEditor.GenericMenu.MenuFunction2 AddSimplexNode = (t) =>{
			...
			//MethodInfo Wrapper
			UnityEditor.GenericMenu.MenuFunction2 AddMethodNode = (object m) =>{
...
/*
			//FieldInfo Wrapper Get
			UnityEditor.GenericMenu.MenuFunction2 AddFieldGetNode = (object f) =>{
				var fieldWrapper = graph.AddNode<ReflectedFieldNodeWrapper>(mousePos);
				fieldWrapper.SetField( (FieldInfo)f, ReflectedFieldNodeWrapper.AccessMode.GetField );
				FinalizeConnection(fieldWrapper);
			};

			//FieldInfo Wrapper Set
			UnityEditor.GenericMenu.MenuFunction2 AddFieldSetNode = (object f) =>{
				var fieldWrapper = graph.AddNode<ReflectedFieldNodeWrapper>(mousePos);
				fieldWrapper.SetField( (FieldInfo)f, ReflectedFieldNodeWrapper.AccessMode.SetField );
				FinalizeConnection(fieldWrapper);
			};
*/
			//Normal FG Node
			GenericMenu.MenuFunction2 AddFlowNode = (t) =>{
			...
			if (clickedPort is ValueOutput){
				var methods = clickedPort.type.GetMethods(flags).ToList();
				...
				foreach (var m in methods.OrderBy(m => !m.IsStatic).OrderBy(m => m.IsSpecialName).OrderBy(m => m.DeclaringType != clickedPort.type ) ){
					...
					menu.AddItem(new GUIContent(name, icon), false, AddMethodNode, m);
				}
/*
				var fields = clickedPort.type.GetFields(flags).ToList();
				foreach(var f in fields){
					if (f.IsObsolete() || f.IsReadOnly()){
						continue;
					}
					var categoryName = clickedPort.type.FriendlyName() + "/";
					if (f.DeclaringType != clickedPort.type){
						categoryName += "More/";
					}
					var name = categoryName + "Set " + f.Name;
					var icon = UnityEditor.EditorGUIUtility.ObjectContent(null, clickedPort.type).image;
					menu.AddItem(new GUIContent(name, icon), false, AddFieldSetNode, f);
				}
*/
			}
			...
			if (clickedPort is FlowOutput || clickedPort is FlowInput){
				foreach(var info in EditorUtils.GetScriptInfosOfType(typeof(FlowControlNode))){
					menu.AddItem(new GUIContent(info.category + "/" + info.name, info.icon, info.description), false, AddFlowNode, info.type );
				}
			}
			...
			foreach (var info in EditorUtils.GetScriptInfosOfType(typeof(SimplexNode))){
				...
				if (clickedPort is ValueOutput){
					...
					if ( parameterTypes.Any(t => t.IsAssignableFrom(clickedPort.type)) ){
						menu.AddItem(new GUIContent(info.category + "/" + info.name, info.icon, info.description), false, AddSimplexNode, info.type);
					}
				}
			...
			if (clickedPort is ValueInput){
				...
				foreach (var type in types){
					foreach (var m in type.GetMethods(flags).OrderBy(m => m.IsSpecialName) ){
						if (m.IsObsolete()){
							continue;
						}
						...
						if (m.ReturnType == typeof(void) || !clickedPort.type.IsAssignableFrom(m.ReturnType) ){
							continue;
						}
...
/*
					...
					foreach(var f in fields){
						...
						var categoryName = "Functions/Reflected/" + type.FriendlyName() + "/";
						var nameForGet = categoryName + (f.IsReadOnly()? "constant " + f.Name : "Get " + f.Name);
						var icon = UnityEditor.EditorGUIUtility.ObjectContent(null, type).image;
						menu.AddItem(new GUIContent(nameForGet, icon), false, AddFieldGetNode, f);
					}
*/
				}
			}
			...
			if (clickedPort is ValueInput){
				...
				if ( clickedPort.type == typeof(GameObject) ){
					menu.AddItem(new GUIContent("Variables/Self"), false, AddFlowNode, typeof(OwnerVariable) );
				}
				...
				menu.AddItem(new GUIContent(string.Format("Variables/Graph Variable ({0})", clickedPort.type.FriendlyName() )), false, ()=>
				{
					...
					AddFlowNode(genericType);
				});

				menu.AddItem(new GUIContent(string.Format("Variables/Blackboard/Get Other Of Type ({0})", clickedPort.type.FriendlyName() )), false, ()=>
				{
				...
				foreach(var pair in variables){
					foreach (var _bbVar in pair.Value){
						...
						if (clickedPort.type.IsAssignableFrom(bbVar.varType)){
							bbVarAdded = true;
							menu.AddItem(new GUIContent(string.Format("Variables/Blackboard/Get {0}", finalName), null, "Variable from Blackboard"), false, ()=>
							{
				...
				if (!bbVarAdded){
					menu.AddDisabledItem(new GUIContent("Variables/Blackboard") );
				}
			}

			if (clickedPort is ValueOutput){

				menu.AddItem(new GUIContent(string.Format("Variables/Blackboard/Set Of Type ({0})", clickedPort.type.FriendlyName() )), false, ()=>
				{
					var genericType = typeof(SetVariable<>).MakeGenericType(new System.Type[]{clickedPort.type});
					AddFlowNode(genericType);
				});

				menu.AddItem(new GUIContent(string.Format("Variables/Blackboard/Set Other Of Type ({0})", clickedPort.type.FriendlyName() )), false, ()=>
				{
				...
				foreach(var pair in variables){
					foreach (var _bbVar in pair.Value){
						...
						if (bbVar.varType.IsAssignableFrom(clickedPort.type)){
							menu.AddItem(new GUIContent(string.Format("Variables/Blackboard/Set {0}", finalName), null, "Variable from Blackboard"), false, ()=>
							{
		...
		protected override GenericMenu OnContextMenu(GenericMenu menu){
			...
			if (type.IsGenericType){
				...
				foreach(var _info in infos){
					var info = _info;
					menu.AddItem(new GUIContent("Change Type/" + info.name), info.type == this.GetType(), ()=>{ this.ReplaceWith(info.type); });
				}
			}
